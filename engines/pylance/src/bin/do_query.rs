...
use pylance_engine::{count_query, open_dataset, run_topk, topk_count};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in stdin.lock().lines() {
        ...
        match parse_command(command) {
            Some(Command::Count) => {
                let result = count_query(&dataset, query).await?;
                writeln!(stdout, "{}", result)?;
            }
            Some(Command::TopK(k)) => {
                run_topk(&dataset, query, k).await?;
                writeln!(stdout, "1")?;
            }
            Some(Command::TopKCount(k)) => {
                let result = topk_count(&dataset, query, k).await?;
                writeln!(stdout, "{}", result)?;
            }
            None => {
                writeln!(stdout, "UNSUPPORTED")?;
